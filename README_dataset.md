...
```
trigger → open_case → gather_evidence → match_patterns → assess (p, evidence count, independence)
   ├─ stop rule met ───────▶ recommend → policy_check → explain → write_case_to_graph
   └─ not met, policy allows a request
          → initial actions recorded → request_evidence → (simulated response)
          → re-assess → final actions → explain → write_case_to_graph
```
